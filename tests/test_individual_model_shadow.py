...
def _canonical_nutrition_record():
    plan = nutrition_plan.build_plan(
        ...
        restrictions=(),
        provenance={"test": "individual-model"},
        language="en",
    )
...
def test_shadow_default_off_skips_builder_and_telemetry(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: built.__setitem__("count", built["count"] + 1),
    )
...
def test_consumer_renders_only_unknown_completion_freshness_through_chat(client, monkeypatch):
    ...
    monkeypatch.setattr(
        appmod.individual_model_snapshot,
        "build_individual_model_snapshot",
        lambda *_args, **_kwargs: _completion_evidence_snapshot(),
    )
    ...
    assert "observed deterministic progression action=" not in addendum
    ...
    assert "Do not alter deterministic plans, safety constraints, progression, or nutrition targets." in addendum
    for forbidden in (
...
def test_consumer_downgrades_unsupported_completion_freshness_to_unknown(
    ...
    monkeypatch.setattr(
        appmod.individual_model_snapshot,
        "build_individual_model_snapshot",
        lambda *_args, **_kwargs: _completion_evidence_snapshot(source_freshness),
    )
...
def test_consumer_rejects_nonproduction_projection_freshness_before_llm(
    ...
    monkeypatch.setattr(
        ...
        "build_projection",
        lambda *_args, **_kwargs: IndividualModelCoachingProjectionV1(
...
def test_optional_context_timeout_preserves_chat_sse_and_drops_projection(client, monkeypatch):
    llm_calls = _mock_stream(monkeypatch)
    _login(client, "consumer-timeout@example.com")
    monkeypatch.setenv("INDIVIDUAL_MODEL_CONSUMER", "true")
    monkeypatch.setattr(shadow, "OPTIONAL_CONTEXT_TIMEOUT_MS", 5)
...
def test_consumer_requires_an_authenticated_account_and_redacts_persisted_identity(
    ...
    def build(user_id, **kwargs):
        ...
        return original_build(user_id, **kwargs)
...
def test_consumer_preserves_persisted_nutrition_targets_and_user_visible_output(
    ...
    llm_calls = _mock_stream(monkeypatch)
    profile = _training_profile()
    plan = _canonical_nutrition_record()
    ...
    assert off_after["version"] == on_after["version"] == plan["version"]
    assert off_after["targets"] == on_after["targets"] == plan["targets"]
    assert "REDACTED INDIVIDUAL MODEL CONTEXT" not in off_prompt
    addendum = _individual_model_addendum(llm_calls[-1])
    assert "authoritative nutrition targets=calories:1260,protein_g:60,carbs_g:120,fat_g:60" in addendum
...
def test_shadow_on_observes_redacted_presence_and_does_not_change_prompt(client, monkeypatch):
    ...
    monkeypatch.setattr(
        appmod.individual_model_snapshot, "build_individual_model_snapshot", lambda *_args, **_kwargs: _snapshot())
    composer_calls = []
    ...
    baseline_user = _login(client, "shadow-baseline@example.com")
    store.add_account_training_constraints(baseline_user, ("vertical_push",))
    off_events = _events(_post(client))
    off_messages = llm_calls[-1]["messages"]

    shadow_user = _login(client, "shadow-on@example.com")
    store.add_account_training_constraints(shadow_user, ("vertical_push",))
    ...
    assert telemetry == {
        ...
        "prescribed_increase_observed": 1,
        "prescribed_maintain_observed": 0,
        "trajectory_insufficient_evidence": 0,
