from datetime import datetime, timezone
from decimal import Decimal
import pytest
from individual_model_projection import (
    IndividualModelPresentationClaimKind,
    IndividualModelPresentationClaimV1,
    IndividualModelCoachingProjectionV1,
    IndividualModelProjectionUnavailable,
    build_presentation_claims,
    build_projection,
    build_request_authority,
    render_prompt,
    validate_presentation_claims,
    validate_projection,
)
from individual_model_snapshot import IndividualModelSnapshotV1
from nutrition_plan import NutritionTargets

def _snapshot(**changes):
    base = dict(schema_version="individual-model-snapshot-v1", user_id="secret", profile={"goal":"strength","level":"beginner","equipment":"home","note":"secret"}, constraints=({"id":"secret","pattern":"vertical_push","source":"x","state":"active"},), training={"latest_authoritative_completed_session_evidence":True,"latest_authoritative_completed_session_freshness":"unknown","latest_authoritative_completed_session_occurred_at":datetime(2026,9,2,10,tzinfo=timezone.utc)}, progression=(), trajectory=({"trajectory_state":"progressing","completion_ids":("secret",)},), adherence="unknown", human_state={"motivation":{"value":"secret"}}, nutrition={"targets":{"calories":2000,"secret":999}}, generated_at=datetime.now(timezone.utc))
    base.update(changes); return IndividualModelSnapshotV1(**base)

def test_projection_is_closed_redacted_and_excludes_hse_ids_and_free_text():
    prompt = render_prompt(build_projection(_snapshot()))
    assert "goal=strength" in prompt
    assert "observed deterministic progression action=increase_was_prescribed" in prompt
    assert "authoritative completed-session evidence freshness=unknown" in prompt
    for forbidden in ("secret", "motivation", "note", "user_id", "adherence", "progressing", "stable", "stalled", "regressing"):
        assert forbidden not in prompt
    assert "recent" not in prompt and "current" not in prompt and "lately" not in prompt
    assert "2026-09-02T10:00:00" not in prompt
    assert "calories:2000" in prompt

def test_insufficient_trajectory_evidence_never_claims_physical_progress():
    prompt = render_prompt(build_projection(_snapshot(trajectory=({"trajectory_state":"insufficient_evidence"},), profile={})))
    assert "observed deterministic progression action=insufficient_evidence" in prompt
    for forbidden in ("progressing", "stable", "stalled", "regressing"):
        assert forbidden not in prompt


def test_completion_ids_or_adherence_never_create_a_completion_claim():
    prompt = render_prompt(build_projection(_snapshot(
        training={"latest_completion_id": "secret", "latest_session_id": "secret"},
        adherence="missed",
    )))

    assert "completed-session" not in prompt
    assert "adherence" not in prompt


@pytest.mark.parametrize("state", ("partial", "skipped", "abandoned", "unknown"))
def test_noncompleted_execution_states_never_create_a_completed_projection_claim(state):
    prompt = render_prompt(build_projection(_snapshot(training={
        "latest_execution_state": state,
        "latest_authoritative_completed_session_evidence": False,
    })))

    assert "completed-session" not in prompt


@pytest.mark.parametrize("freshness", ("current", "stale", "invalid"))
def test_no_unsupported_freshness_state_can_reach_the_projection(freshness):
    prompt = render_prompt(build_projection(_snapshot(training={
        "latest_authoritative_completed_session_evidence": True,
        "latest_authoritative_completed_session_freshness": freshness,
        "latest_authoritative_completed_session_occurred_at": datetime(2026, 9, 2, tzinfo=timezone.utc),
    })))

    assert "freshness=unknown" in prompt
    assert f"freshness={freshness}" not in prompt


def test_projection_validator_rejects_nonproduction_freshness_states():
    with pytest.raises(ValueError):
        validate_projection(IndividualModelCoachingProjectionV1(
            None, None, None, (), "current", None, ()))


def test_current_request_authority_wins_over_historical_snapshot_context():
    snapshot = _snapshot(
        constraints=({"pattern": "squat"},),
        nutrition={"targets": {"calories": "1800", "protein_g": "120"}},
    )
    authority = build_request_authority(
        constraint_store_available=True,
        active_training_constraints=("vertical_push",),
        current_nutrition_targets=NutritionTargets(
            kcal=Decimal("2200"), protein=Decimal("160"), carbs=Decimal("240"), fat=Decimal("70")),
        nutrition_authority_resolved=True,
    )

    prompt = render_prompt(build_projection(snapshot, request_authority=authority))

    assert "active movement exclusions=vertical_push" in prompt
    assert "squat" not in prompt
    assert "authoritative nutrition targets=calories:2200,protein_g:160,carbs_g:240,fat_g:70" in prompt
    assert "1800" not in prompt


def test_unavailable_current_constraint_store_drops_the_optional_projection():
    authority = build_request_authority(
        constraint_store_available=False,
        active_training_constraints=(),
    )

    with pytest.raises(IndividualModelProjectionUnavailable):
        build_projection(_snapshot(), request_authority=authority)


def test_decimal_and_string_nutrition_values_are_normalized_only_through_closed_fields():
    prompt = render_prompt(build_projection(_snapshot(nutrition={"targets": {
        "calories": "2200", "protein_g": Decimal("160.5"), "carbs_g": "240", "fat_g": "70",
        "free_text": "secret",
    }})))

    assert "authoritative nutrition targets=calories:2200,protein_g:160.5,carbs_g:240,fat_g:70" in prompt
    assert "secret" not in prompt


@pytest.mark.parametrize("targets", (
    {"calories": "not-a-number"},
    {"calories": 2200, "protein_g": "NaN"},
    {"calories": 0},
))
def test_malformed_nutrition_targets_fail_closed(targets):
    prompt = render_prompt(build_projection(_snapshot(nutrition={"targets": targets})))

    assert "authoritative nutrition targets=" not in prompt


def test_presentation_claims_are_closed_and_cannot_be_forged_as_authority():
    claims = build_presentation_claims(build_projection(_snapshot()))
    assert all(isinstance(claim.kind, IndividualModelPresentationClaimKind) for claim in claims)
    with pytest.raises(ValueError):
        validate_presentation_claims((IndividualModelPresentationClaimV1(
            IndividualModelPresentationClaimKind.GOAL, "private-goal"),))
